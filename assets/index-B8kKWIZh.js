...
`+JSON.stringify(window.problems120C)),i.classList.remove("hidden"),i.innerHTML=`
        ...
        `,typeof confetti=="function"&&confetti({particleCount:150,spread:70}))},submitProgress(e){if(window.submitMathLesson){const t=`Trắc nghiệm tổng: Đúng ${x.correctCount}/${x.currentQuiz.length} câu. (Củng cố: ${x.score})`;window.submitMathLesson(t,x.score,"btn-submit-final",e,x.currentQuiz.length,x.correctCount)}},loadLeaderboard(e){var o;if(!window.db||!e)return;const t=document.getElementById("quiz-leaderboard-list");if(!t)return;const n=String(e.period||e.id),i=((o=window.router)==null?void 0:o.currentSubject)||"vietnamese",s=i==="math"?"Toán học":i==="history"?"Lịch sử - Địa lý":"Tiếng Việt";x._leaderboardUnsub&&x._leaderboardUnsub(),t.innerHTML='<div class="text-center py-8 text-slate-500"><i class="fas fa-spinner fa-spin text-2xl mb-2 opacity-30"></i><p class="text-[10px] font-black uppercase tracking-widest">Đang nạp bảng vàng...</p></div>',x._leaderboardUnsub=window.db.collection("diem_tieng_viet_lop5").where("period","==",n).where("subject","==",s).onSnapshot(a=>{var m;if(a.empty){t.innerHTML=`
                    ...
                    `;return}let r=[];a.forEach(p=>r.push(p.data())),r.sort((p,b)=>{var L,B;const w=(L=p.timestamp)!=null&&L.toDate?p.timestamp.toDate():0;return((B=b.timestamp)!=null&&B.toDate?b.timestamp.toDate():0)-w});const l=r.slice(0,10),d=Math.min(5,((m=e.exercises)==null?void 0:m.length)||3),c=["📚","📝","✅","🖊️","🌟"],h=["text-blue-600","text-emerald-600","text-orange-600","text-rose-500","text-purple-500"],u=p=>p==null||typeof p=="number"&&p===0?'<span class="text-slate-300 text-xs">-</span>':p>=100?'<span class="text-base text-emerald-500">✅</span>':'<span class="text-base text-red-500">❌</span>',g=p=>{if(!p)return"00:00";const b=Math.floor(p/60),w=p%60;return`${b.toString().padStart(2,"0")}:${w.toString().padStart(2,"0")}`};t.innerHTML=`
                    <div class="overflow-x-auto -mx-4 md:mx-0">
                        <table class="w-full text-left border-collapse min-w-[650px]">
                            ...
                            <tbody class="divide-y divide-white/5">
                                ${l.map((p,b)=>{const w=Array.from({length:d}).map((M,U)=>p[`scoreEx${U+1}`]),I=p.scoreQuiz||p.score||0,L=e.exercises?e.exercises.length:3;let B=0;for(let M=1;M<=L;M++)B+=p[`scoreEx${M}`]||0;const V=Math.round(B/L),q=Math.round((V+I)/2);return`
                                        <tr class="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                                            ...
                                            <td class="px-2 py-3.5 text-center text-[11px] font-black text-indigo-400">${I}%</td>
...
`);this.data=[];for(let i of n){if(i=i.trim(),!i||i.startsWith("//")||i.startsWith("#")||i.toUpperCase().startsWith("ID"))continue;const s=i.split("|").map(u=>u.trim());if(s.length<5)continue;const o=s[0],a=s[1],r=isNaN(a)?a:parseInt(a)||0,l=parseInt(s[2])||1,d=s[3],c=s[4],h=s[5]?s[5].split(",").map(u=>u.trim()).filter(u=>u):[];this.data.push({id:o,period:r,level:l,q:d,a:c,customDistractors:h})}},generateDistractors(e,t){let n=new Set,i=e.match(/^([\d.,]+)\s*(.*)$/),s=e.match(/(\d+)\s*([a-zA-Záàãảạăắằẵẳặâấầẫẩậéèẽẻẹêếềễểệíìĩỉịóòõỏọôốồỗổộơớờỡởợúùũủụưứừữửựýỳỹỷỵđ]+)\s*(\d+)?\s*([a-zA-Záàãảạăắằẵẳặâấầẫẩậéèẽẻẹêếềễểệíìĩỉịóòõỏọôốồỗổộơớờỡởợúùũủụưứừữửựýỳỹỷỵđ]+)?/i);if(i&&!s){const a=i[1].replace(",","."),r=parseFloat(a),l=i[2]||"";if(!isNaN(r)){const d=[r*10,r/10,r+1,r-1,r+10,r-10,r+.5,r-.5,parseFloat(a.split("").reverse().join(""))].filter(c=>c>0&&c!==r).map(c=>{let h=Number.isInteger(c)?c.toString():c.toFixed(1).replace(".0","");return l?`${h} ${l}`:h});d.sort(()=>Math.random()-.5);for(let c of d)if(c!==e&&n.add(c),n.size>=3)break}}if(n.size<3&&s&&s[3]){let a=parseInt(s[1]),r=s[2],l=parseInt(s[3]),d=s[4]||"";const c=`${a+1} ${r} ${l} ${d}`.trim(),h=`${a} ${r} ${l+10} ${d}`.trim(),u=`${a-1>0?a-1:a+2} ${r} ${l} ${d}`.trim(),g=`${l} ${r} ${a} ${d}`.trim();[c,h,u,g].forEach(m=>{m!==e&&n.add(m)})}let o=[...t].filter(a=>a!==e&&a.length>0);o.sort(()=>Math.random()-.5);for(let a of o){if(n.size>=3)break;n.add(a)}for(;n.size<3;){let a=Math.floor(Math.random()*100)+1;!n.has(a.toString())&&a.toString()!==e&&n.add(a.toString())}return Array.from(n).slice(0,3)},async getQuestions(e={}){await this.init();let t=this.data;if(e.periods){const s=Array.isArray(e.periods)?e.periods:[e.periods];t=t.filter(o=>s.includes(o.period))}let n=[];if(e.countsPerLevel)for(let[s,o]of Object.entries(e.countsPerLevel)){let a=t.filter(r=>r.level==parseInt(s));a.sort(()=>Math.random()-.5),n=n.concat(a.slice(0,o))}else{let s=e.totalCount||10;n=[...t].sort(()=>Math.random()-.5).slice(0,s)}const i=Array.from(new Set(t.map(s=>s.a)));return n.map(s=>{let o=[...s.customDistractors];if(o.length<3){let l=this.generateDistractors(s.a,i);for(let d of l){if(o.length>=3)break;o.includes(d)||o.push(d)}}let a=[s.a,...o].slice(0,4);a.sort(()=>Math.random()-.5);let r=a.indexOf(s.a);return{id:s.id,period:s.period,level:s.level,question:s.q,options:a,answer:r}})},async getQuizPool(e){await this.init();const t=String(e);if(this._quizPoolCache[t])return this._quizPoolCache[t];const n=this.data.filter(s=>String(s.period)===t);if(n.length===0)return[];const i=this._toQuizPool(n);return this._quizPoolCache[t]=i,i},_toQuizPool(e){const t=Array.from(new Set(e.map(n=>n.a)));return e.map(n=>{let i=[...n.customDistractors];if(i.length<3){const a=this.generateDistractors(n.a,t);for(const r of a){if(i.length>=3)break;!i.includes(r)&&r!==n.a&&i.push(r)}}let s=[n.a,...i.filter(a=>a!==n.a)].slice(0,4);s.sort(()=>Math.random()-.5);const o=s.indexOf(n.a);return{question:n.q,options:s,answer:o}})},async hasPeriod(e){return await this.init(),this.data.some(t=>String(t.period)===String(e))},async getPeriodStats(){await this.init();const e={};return this.data.forEach(t=>{const n=String(t.period);e[n]=(e[n]||0)+1}),e}};window.QuestionBankManager=se;const _e={renderModal(){let e=document.getElementById("qb-admin-modal");e||(e=document.createElement("div"),e.id="qb-admin-modal",e.className="fixed inset-0 z-[2147483647] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 transition-opacity duration-300 pointer-events-auto",document.body.appendChild(e)),e.classList.remove("hidden"),e.innerHTML=`
        ...
        `},initBookReader(e){var d,c,h,u,g,m,p,b,w,I,L,B,V,q,M,U;const t=this,n=document.getElementById("book-toc-drawer"),i=document.getElementById("book-wrapper-scale"),s=document.getElementById("book-element"),o=()=>{const f=document.getElementById("input-current-page");f&&(f.value=t.state.currentPage);const C=document.getElementById("img-page-left"),v=document.getElementById("img-page-right"),T=String(t.state.currentPage).padStart(3,"0"),N=String(t.state.currentPage+1).padStart(3,"0");C&&(C.src=`assets/images/sgk-lsdl/SGK_lich-su-va-dia-li-lop-5-kntt-da_nen_011f5-${T}.png`),v&&t.state.viewMode==="double"&&(v.src=`assets/images/sgk-lsdl/SGK_lich-su-va-dia-li-lop-5-kntt-da_nen_011f5-${N}.png`),i&&(i.style.transform=`scale(${t.state.zoom/100})`);const y=t.getCurrentLesson(t.state.currentPage),H=document.getElementById("hotspot-left");H&&(y&&y.objectives&&y.startPage===t.state.currentPage?H.classList.remove("hidden"):H.classList.add("hidden"));const j=t.getCurrentLesson(t.state.currentPage+1),P=document.getElementById("hotspot-right");P&&t.state.viewMode==="double"&&(j&&j.objectives&&j.startPage===t.state.currentPage+1?P.classList.remove("hidden"):P.classList.add("hidden"))},a=()=>{const f=t.state.viewMode==="double"?2:1;t.state.currentPage<t.state.totalPages-(t.state.viewMode==="double"?1:0)&&(s&&s.classList.add("flipped"),setTimeout(()=>{t.state.currentPage+=f,o(),s&&s.classList.remove("flipped")},300))},r=()=>{const f=t.state.viewMode==="double"?2:1;t.state.currentPage>1&&(s&&s.classList.add("flipped"),setTimeout(()=>{t.state.currentPage=Math.max(1,t.state.currentPage-f),o(),s&&s.classList.remove("flipped")},300))};(d=document.getElementById("btn-page-next"))==null||d.addEventListener("click",a),(c=document.getElementById("btn-page-prev"))==null||c.addEventListener("click",r),(h=document.getElementById("corner-next-top"))==null||h.addEventListener("click",f=>{f.stopPropagation(),a()}),(u=document.getElementById("corner-next-bottom"))==null||u.addEventListener("click",f=>{f.stopPropagation(),a()}),(g=document.getElementById("corner-prev-top"))==null||g.addEventListener("click",f=>{f.stopPropagation(),r()}),(m=document.getElementById("corner-prev-bottom"))==null||m.addEventListener("click",f=>{f.stopPropagation(),r()}),(p=document.querySelector(".book-container"))==null||p.addEventListener("click",f=>{if(f.target.closest(".objectives-clickbox")||f.target.closest(".quiz-overlay-btn")||f.target.closest("button")||f.target.closest("input"))return;const C=f.currentTarget.getBoundingClientRect(),v=f.clientX-C.left,T=C.width;v<T/2?r():a()}),(b=document.getElementById("btn-zoom-in"))==null||b.addEventListener("click",()=>{t.state.zoom=Math.min(150,t.state.zoom+10),document.getElementById("zoom-value").innerText=`${t.state.zoom}%`,o()}),(w=document.getElementById("btn-zoom-out"))==null||w.addEventListener("click",()=>{t.state.zoom=Math.max(50,t.state.zoom-10),document.getElementById("zoom-value").innerText=`${t.state.zoom}%`,o()}),(I=document.getElementById("btn-toc-toggle"))==null||I.addEventListener("click",()=>{n==null||n.classList.toggle("open")}),n==null||n.querySelectorAll(".toc-item").forEach(f=>{f.addEventListener("click",()=>{const C=parseInt(f.getAttribute("data-page"),10);t.state.currentPage=t.state.viewMode==="double"&&C%2===0?C-1:C,o(),n.classList.remove("open")})}),(L=document.getElementById("btn-viewmode-toggle"))==null||L.addEventListener("click",()=>{t.state.viewMode=t.state.viewMode==="double"?"single":"double";const f=document.getElementById("book-wrapper-scale");t.state.viewMode==="single"?f==null||f.classList.add("single-view"):f==null||f.classList.remove("single-view"),o()}),(B=document.getElementById("input-current-page"))==null||B.addEventListener("keypress",f=>{if(f.key==="Enter"){const C=parseInt(f.target.value,10);!isNaN(C)&&C>=1&&C<=t.state.totalPages&&(t.state.currentPage=t.state.viewMode==="double"&&C%2===0?C-1:C,o())}});const l=f=>{const C=t.getCurrentLesson(f);if(!C||!C.objectives)return;const v=document.getElementById("modal-objectives"),T=document.getElementById("obj-lesson-title"),N=document.getElementById("obj-list-container");v&&T&&N&&(T.innerText=C.title,N.innerHTML=`
        ...
        `;a.innerHTML=d,window.scrollTo(0,0)},handleSearch(e){const t=this.currentSubject,n=_.find(r=>r.id===t),i=this.isAdmin(),o=(z[t]||[]).filter(r=>i||!r.hidden).filter(r=>{const l=e.toLowerCase().trim();return r.title.toLowerCase().includes(l)||r.period.toLowerCase().includes(l)||`tuần ${r.week}`.toLowerCase().includes(l)}),a=document.getElementById("dashboard-lesson-list");a&&(a.innerHTML=k.renderLessonList(o,n.color))},async renderLesson(e,t,n=!1){if(!n){window.location.hash=`/lesson/${e}/${t}`;return}const i=_.find(c=>c.id===e);let s=(z[e]||[]).find(c=>String(c.id)===String(t)||String(c.period)===String(t));(!i||!s)&&(console.warn(`[Router] Lesson not found in manifest: ${e}/${t}. Trying fuzzy match...`),s=(z[e]||[]).find(c=>String(c.id).toLowerCase()===String(t).toLowerCase()||String(c.period).toLowerCase()===String(t).toLowerCase()));const o=this.isAdmin(),a=`${e}_${(s==null?void 0:s.period)||t}`,r=((s==null?void 0:s.hidden)||this.contentLocks.lessons[a])&&!o;if(!i||!s||r){if(console.error(`[Router] Error: Access denied or lesson not found ${e}/${t}`),r)alert("Bài học này hiện đang bị khóa. Vui lòng quay lại sau!"),this.goHome();else{const c=document.getElementById("app-content");c&&(c.innerHTML=`
            ...
            `);try{(!window.__lessonCache||typeof window.__lessonCache.has!="function")&&(window.__lessonCache=new Map);const c=window.__lessonCache,h=`${e}_${t}`;if(c.has(h))Object.assign(s,c.get(h));else if(!s.content){const u=String(s.period).padStart(3,"0"),g=[];if(s.modulePath){const b=s.modulePath.replace("./","").replace(/\.js$/i,"");g.push(`/js/data/${e}/${b}.js`)}g.push(`/js/data/${e}/${u}/index.js`),g.push(`/js/data/${e}/${u}.js`);let m=null;for(const b of g)try{const w=b+`?v=${Date.now()}`;if(console.log(`[Router] Loading: ${w}`),m=await import(w),m)break}catch{}let p=null;if(m){const b=`lesson${e==="math"?s.period:u}`;p=s.exportName&&m[s.exportName]||m.lessonData||m[b]||m.default||Object.values(m)[0]}if(!p){console.warn(`[Router] Physical module missing for ${e}/${t}, fetching placeholders...`);try{const b=await ve(Object.assign({"../data/math/placeholders.js":()=>ne(()=>import("./placeholders-Bm95jhxt.js"),[]),"../data/science/placeholders.js":()=>ne(()=>import("./placeholders-Czdk4T-k.js"),[]),"../data/vietnamese/placeholders.js":()=>ne(()=>import("./placeholders-Du637W2M.js"),[])}),`../data/${e}/placeholders.js`,4),w=b.placeholderLessons||b[`${e.toUpperCase()}_PLACEHOLDERS`];w&&(p=Array.isArray(w)?w.find(I=>String(I.period)===String(t)):w[t])}catch(b){console.error(`[Router] Failed to load placeholders for ${e}:`,b)}}if(p)Object.assign(s,p),c.set(h,p);else throw new Error(`Không thể tìm thấy nội dung bài học: ${e}/${t}`)}this.runPrefetch(e,s)}catch(c){console.error(`[Router] Critical loading error for ${e}/${t}:`,c),l&&(l.innerHTML=`
                ...
                `);const u=document.getElementById("menu-subjects-container");u&&(u.classList.remove("hidden"),u.innerHTML=k.renderNavbarSubjectSwitcher(i,_)),l.innerHTML=k.renderLessonPage(i,s,_),window.scrollTo(0,0);const g=!!(typeof s.content=="function"||typeof s.content=="string"&&s.content.trim()),m=!!(typeof s.practice=="function"||typeof s.practice=="string"&&s.practice.trim()||typeof s.practice=="object"&&s.practice!==null&&Object.keys(s.practice).length>0),p=s.quizPool&&s.quizPool.length>0||await se.hasPeriod(s.period||s.id);let b="study";g?b="study":m?b="practice":p&&(b="quiz"),this.switchTab(b)}},runPrefetch(e,t){const n=z[e]||[],i=n.findIndex(a=>String(a.period)===String(t.period));if(i<0)return;const s=a=>{const r=n[i+a];if(!r||r.content)return;const l=window.__lessonCache,d=`${e}_${r.period}`;if(l&&!l.has(d)){const c=String(r.period).padStart(3,"0");let h;r.modulePath?h=`../data/${e}/${r.modulePath.replace("./","")}`:h=`../data/${e}/${c}/index.js`,import(h+`?v=${Date.now()}`).then(g=>{const m=`lesson${e==="math"?r.period:c}`,p=r.exportName&&g[r.exportName]||g.lessonData||g[m]||g.default||Object.values(g)[0];p&&l.set(d,p)}).catch(()=>{})}},o=()=>{s(1),s(2)};"requestIdleCallback"in window?requestIdleCallback(o):setTimeout(o,1e3)},goHome(){window.location.hash="/"},async switchTab(e){var c,h,u,g,m,p,b,w,I,L,B,V,q,M,U,f,C;_.find(v=>v.id===this.currentSubject);const t=z[this.currentSubject]||[],n=(v,T)=>{const N=String(v).toLowerCase().replace("vn-","").trim(),y=String(T).toLowerCase().replace("vn-","").trim();if(N===y)return!0;const H=N.split("-"),j=y.split("-");return!!H.some(P=>j.includes(P))},i=(v,T)=>{const N=String(v).toLowerCase().replace("vn-","").trim(),y=String(T).toLowerCase().replace("vn-","").trim();return N===y};let s=t.find(v=>i(v.period,this.currentLessonPeriod)||i(v.id,this.currentLessonPeriod));if(s||(s=t.find(v=>n(v.period,this.currentLessonPeriod)||n(v.id,this.currentLessonPeriod))),!s&&window.currentLessonData&&(i(window.currentLessonData.period,this.currentLessonPeriod)||i(window.currentLessonData.id,this.currentLessonPeriod)||n(window.currentLessonData.period,this.currentLessonPeriod)||n(window.currentLessonData.id,this.currentLessonPeriod))&&(s=window.currentLessonData),!s){console.error("Không tìm thấy dữ liệu bài học để chuyển tab");return}e!=="quiz"&&k&&k.cleanupQuiz&&k.cleanupQuiz(),document.querySelectorAll(".tab-btn").forEach(v=>{v.classList.remove("active","text-red-600"),v.classList.add("text-gray-400")});const o=document.getElementById(`tab-${e}`);o&&(o.classList.add("active","text-red-600"),o.classList.remove("text-gray-400")),s&&s.metadata&&(window.MATH_BUILDER_METADATA=s.metadata);const a=document.getElementById("tab-content");if(!a)return;let r="";if(e==="study")if(this.currentSubject==="history")r=k.renderBookReader(s);else{const T=(typeof s.content=="function"?s.content(k):s.content)||((g=(u=(h=(c=s.tabs)==null?void 0:c.study)==null?void 0:h.blocks)==null?void 0:u.find(N=>N.type==="html"))==null?void 0:g.content)||((w=(b=(p=(m=s.tabs)==null?void 0:m.lesson)==null?void 0:p.blocks)==null?void 0:b.find(N=>N.type==="html"))==null?void 0:w.content)||((L=(I=s.blocks)==null?void 0:I.find(N=>N.type==="html"))==null?void 0:L.content)||k.renderEmptyContent();r=`
                    <div class="glass-card rounded-[40px] p-8 md:p-12 bg-white dark:bg-slate-900 min-h-[200px] animate-fade-in text-gray-700 dark:text-slate-200">
                        <div id="lesson-study-container" class="lesson-body">${k.parseTutor(T)}</div>
                    </div>
                `}else if(e==="practice"){const T=(typeof s.practice=="function"?s.practice(k):s.practice)||((M=(q=(V=(B=s.tabs)==null?void 0:B.practice)==null?void 0:V.blocks)==null?void 0:q.find(N=>N.type==="html"))==null?void 0:M.content)||k.renderEmptyPractice();r=`
                <div class="glass-card rounded-[40px] p-8 md:p-12 bg-white dark:bg-slate-900 min-h-[200px] animate-fade-in text-gray-700 dark:text-slate-200">
                    <div id="lesson-practice-container" class="space-y-6">
                        ${k.parseTutor(T)}
                    </div>
                </div>
            `}else if(e==="quiz"){const v=((f=(U=s.tabs)==null?void 0:U.quiz)==null?void 0:f.questions)||((C=s.quiz)==null?void 0:C.questions);if(!s.quizPool&&v&&Array.isArray(v)&&(s.quizPool=v.map(T=>({question:T.q,options:T.a,answer:T.c,level:T.level||1}))),(!s.quizPool||s.quizPool.length===0)&&this.currentSubject==="math"){const T=await se.getQuizPool(s.period||s.id);T.length>0&&(s.quizPool=T,window.currentLessonData&&(window.currentLessonData.quizPool=T))}r=k.renderQuizContainer(s)}else if(e==="pptx"){const v=s.pptLink||"";r=`
            ...
            `}if(a.innerHTML=r,e==="study"&&this.currentSubject==="history"&&k&&typeof k.initBookReader=="function"&&k.initBookReader(s),e==="practice"||e==="study"){const v=a.querySelector("#ltTrack"),T=a.querySelectorAll(".ltTrang");if(v&&T.length>0&&!a.querySelector("#ltNav")){const N=T.length;let y="";for(let S=0;S<N;S++)y+=`
                <div class="${S===0?"ltCham active":"ltCham"}" data-index="${S}" onclick="window.ltChuyenNhanh(${S})" style="width:0.6rem;height:0.6rem;border-radius:50%;background:${S===0?"#2563eb":"#bfdbfe"};transform:${S===0?"scale(1.35)":"scale(1)"};transition:all 0.25s;cursor:pointer;"></div>`;y+=`
            `;const H=`<div id="ltNav" style="position:sticky; bottom:0; display:flex; align-items:center; justify-content:center; gap:1.5rem; padding:0.75rem 1rem; background:rgba(255,255,255,0.95); backdrop-filter:blur(12px); border-top:1px solid rgba(59,130,246,0.12); z-index:50; box-shadow:0 -4px 20px rgba(0,0,0,0.06);">

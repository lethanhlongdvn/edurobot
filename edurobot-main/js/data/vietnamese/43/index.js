...
window.setCheChuVn43 = function(mode) {
    ...
    if (!poemContainer.dataset.tokenized) {
        ...
        paragraphs.forEach(p => {
            ...
            const newHTML = words.map(word => {
                return `<span class="vn43-word transition-all duration-300 cursor-pointer hover:bg-sky-100/50 rounded px-0.5" onclick="if (this.style.color === 'transparent') { this.style.color = 'inherit'; this.style.backgroundColor = 'transparent'; event.stopPropagation(); }">${word}</span>`;
            }).join(' ');
            p.innerHTML = newHTML;
        });
    ...
    if (mode === 'none') {
        ...
        fb.innerHTML = "👀 Đã che toàn bộ bài thơ! Em hãy tự đọc thuộc lòng nhé (Nhấp vào ô che để xem từ gợi ý).";

/**
 * History & Geography Dashboard Logic
 * Handles data fetching, processing, and rendering for teacher-lichsu.html
 */

import { parseScore, formatTimeTaken } from './utils.js';
import { historyManifest } from '../data/history/manifest.js';
...
const SUBJECT_NAME = "Lịch sử - Địa lý";

/**
 * Initializes History-specific filters from manifest.
 */
export function populateFilters() {
    const weekSelect = document.getElementById('week-filter');
    if (!weekSelect) return;
    const weeks = new Set();
    historyManifest.forEach(m => {
...
export function updatePeriodDropdown(selectedWeek) {
    ...
    const periods = historyManifest.filter(m => !selectedWeek || String(m.week) === String(selectedWeek));
    periods.forEach(m => {
        let opt = document.createElement('option');
        opt.value = m.period;
...
async function getLessonExpectedExKeys(periodId) {
    if (!periodId) return [];
    const lessonMeta = historyManifest.find(m => String(m.period) === String(periodId));
    if (!lessonMeta || !lessonMeta.modulePath) return [];
    try {
        const basePath = '../data/history/';
...
/**
 * Initializes Real-time Firebase listeners for History & Geography.
 */
export function initRealtime() {
    ...
    // 1. Củng cố - chỉ lọc Lịch sử & Địa lý
    ...
    // 2. Luyện tập Tự luận - chỉ lọc Lịch sử & Địa lý
...
/**
 * Downloads history scores as Excel file.
 */

/**
 * Vietnamese Dashboard Logic
 * Handles data fetching, processing, and rendering for teacher-tiengviet.html
 */

import { parseScore, formatTimeTaken } from './utils.js';
import { vietnameseManifest } from '../data/vietnamese/manifest.js';
...
/**
 * Initializes Vietnamese-specific filters from manifest.
 */
export function populateFilters() {
    const weekSelect = document.getElementById('week-filter');
    if (!weekSelect) return;
    const weeks = new Set();
    vietnameseManifest.forEach(m => {
...
export function updatePeriodDropdown(selectedWeek) {
    ...
    const periods = vietnameseManifest.filter(m => !selectedWeek || String(m.week) === String(selectedWeek));
    periods.forEach(m => {
        let opt = document.createElement('option');
        opt.value = m.period;
...
async function getLessonExpectedExKeys(periodId) {
    if (!periodId) return [];
    const lessonMeta = vietnameseManifest.find(m => String(m.period) === String(periodId));
    if (!lessonMeta || !lessonMeta.modulePath) return [];
    try {
        const basePath = '../data/vietnamese/';
...
/**
 * Initializes Real-time Firebase listeners for Vietnamese.
 */
...
export function renderStats() {
    ...
    try {
        ...
        // Render UI using shared Table module
...
/**
 * Downloads vietnamese scores as Excel file (proxy to shared module).
 */
export function exportToExcel() {
    excelExport(allQuizData, "Tiếng Việt");
}

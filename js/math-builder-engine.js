...
window.showMathFeedback = function(isCorrect, rightAnswer, studentAnswer, guidance, solution) {
  ...
  if (isCorrect) {
    ...
    // Auto-split sub-points onto new lines for neat rendering
    solutionHtml = solutionHtml.replace(/(?<!<br>|^)\s*-\s+/g, '<br>- ');
    solutionHtml = solutionHtml.replace(/(?<!<br>|^)\s*\b([a-z])\)\s+/gi, '<br>$1) ');
    solutionHtml = solutionHtml.replace(/(?<!<br>|^)\s*\b(\d+)\.\s+/g, '<br>$1. ');
    ...
    let guidanceHtml = (guidance || 'Hãy kiểm tra kỹ các bước tính và đơn vị em nhé! Thử lại một lần nữa nào.').replace(/\\n/g, '<br>').replace(/\n/g, '<br>');
    // Auto-split sub-points onto new lines for guidance
    guidanceHtml = guidanceHtml.replace(/(?<!<br>|^)\s*-\s+/g, '<br>- ');
    guidanceHtml = guidanceHtml.replace(/(?<!<br>|^)\s*\b([a-z])\)\s+/gi, '<br>$1) ');
    guidanceHtml = guidanceHtml.replace(/(?<!<br>|^)\s*\b(\d+)\.\s+/g, '<br>$1. ');
    body.innerHTML = '<div class="space-y-4">' +
                        ...
                        '<div class="bg-amber-50 p-6 rounded-3xl border-2 border-amber-100 shadow-inner">' +
                          ...
                          '</p>' +
                          '<div class="text-slate-700 text-3xl leading-relaxed">' + guidanceHtml + '</div>' +
                        '</div>' +
...
window.closeMathModal = function() {
  ...
  renderCheckBtn(fnName, accent) {
    return `<button onclick="${fnName}()" class="w-14 h-14 md:w-20 md:h-20 bg-orange-600 hover:bg-orange-700 text-white rounded-full font-black text-2xl md:text-4xl flex items-center justify-center shadow-lg active:scale-95 transition-all">E</button>`;
  },

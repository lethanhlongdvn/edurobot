...
window.submitVn27Global = function() {
    if (typeof window.showMathFeedback === 'function') {
        window.showMathFeedback(
            'Hoàn thành Tiết 27',
            '📝',
            '<div class="text-center p-6"><span class="text-7xl block mb-4">📝</span><p class="text-lg md:text-2xl font-bold text-sky-800">Chúc mừng em đã hoàn thành bài học Đánh giá, chỉnh sửa báo cáo công việc!</p><p class="text-2xl md:text-3xl text-gray-800 mt-3">Một thái độ cầu tiến lắng nghe góp ý và sự tỉ mỉ đọc soát lỗi là chìa khóa giúp bài viết của em ngày càng hoàn hảo và chuyên nghiệp hơn.</p></div>'
        );
    }
};

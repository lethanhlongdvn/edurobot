...
export const LessonCore = {
    ...
    submitEssayAI(id, questionText) {
        const input = document.getElementById(`essay-input-${id}`);
        const solution = input ? input.value : '';
        
        if (!solution.trim()) { 
            alert("Thầy E nhắc: Em hãy viết câu trả lời trước khi nộp bài nhé! ✏️"); 
            return; 
        }

        const displayPrompt = `Câu hỏi:\n${questionText}\n\nBài làm của học sinh:\n${solution}`;
...
5. Format nội dung đẹp, dễ đọc.`;

        if (window.AIInteraction && typeof window.AIInteraction.gradeWithModal === 'function') {
            // Callback để lưu dữ liệu sau khi AI chấm xong
            const onGradeComplete = (score, aiResponse) => {
                if (window.submitMathLesson) {
                    // Trích xuất số thứ tự bài từ ID (vd: 069-1 -> bài 1)
                    const exNumMatch = id.match(/-(\d+)$/);
                    const exNum = exNumMatch ? exNumMatch[1] : "1";
                    
                    const saveContent = `Bài ${exNum}. ${questionText}\n\nBài làm: ${solution}`;
                    
                    // Gọi hàm nộp bài (Hệ thống sẽ tự nhận diện Bài N để lưu scoreExN)
                    window.submitMathLesson(saveContent, score, null, 0);
                    console.log(`[LessonCore] Đã gửi yêu cầu lưu bài tự luận ${exNum} với điểm: ${score}`);
                }
            };

            window.AIInteraction.gradeWithModal("👨‍🏫 Thầy E Nhận Xét & Chấm Điểm", displayPrompt + hiddenPrompt, onGradeComplete);
        }
    },

    renderLessonPage(subject, lesson, allSubjects = []) {
        const hasStudy = !!(typeof lesson.content === 'function' || (typeof lesson.content === 'string' && lesson.content.trim()) || (Array.isArray(lesson.tabs) && lesson.tabs.some(t => t.id === 'study')));
        const hasPractice = !!(typeof lesson.practice === 'function' || (typeof lesson.practice === 'string' && lesson.practice.trim()) || (typeof lesson.practice === 'object' && lesson.practice !== null && Object.keys(lesson.practice).length > 0) || (Array.isArray(lesson.tabs) && lesson.tabs.some(t => t.id === 'practice')));
        const hasQuiz = !!(lesson.quizPool || (lesson.metadata && lesson.metadata.quizPool) || lesson.quizFile || lesson.period === "156" || (Array.isArray(lesson.tabs) && lesson.tabs.some(t => t.id === 'quiz')));
                    ...
                    <div id="ai-chat-messages" class="flex-grow p-5 md:p-6 overflow-y-auto space-y-4 bg-gray-50/50 dark:bg-slate-900/50" style="min-height:0">
                        <div class="flex mb-3 animate-slide-up">
                            <div class="bg-white dark:bg-slate-800 border border-blue-50 dark:border-slate-700 p-4 rounded-2xl rounded-tl-none max-w-[85%] text-sm text-gray-700 dark:text-slate-200 font-medium shadow-sm leading-relaxed">
                                ...
                                <p class="mb-2">Chào bạn! Mình là **Robot**, mình rất vui được hỗ trợ bạn trong bài học <b>${lesson.title}</b> này.</p>

            ...
            <div class="flex flex-col mb-3">
                <div class="conv-exercise py-2 flex items-center justify-between gap-3 group" data-exercise-id="${id}">
                    ...
                    <div class="relative flex-grow flex items-center gap-3 justify-end">
                        ...
                        <span class="font-bold text-blue-600 text-base md:text-lg shrink-0 min-w-[2rem] text-left">${unit}</span>
                ...
                <div class="flex flex-col md:flex-row md:items-center gap-4 md:gap-6">
                    <div class="flex items-center gap-4 md:gap-5 shrink-0">
                        ...
                        <div>
                            ...
                            <h4 class="text-3xl md:text-4xl font-black text-blue-600">${displayValue}</h4>
                        </div>
                    </div>
    ...
    renderVerticalCalculation(id, num1, operator, num2, answer, decimalPlaces = 0) {
        ...
        const renderDisplayRow = (numStr, prefix = '') => {
            ...
            for (let i = 0; i < maxLen; i++) {
                const ch = numStr[i] || '';
                const display = ch === ' ' ? '' : ch;
                cells += `<div class="w-10 h-10 md:w-12 md:h-12 border border-blue-200 bg-white flex items-center justify-center text-lg md:text-xl font-black text-blue-600">${display}</div>`;
            }
...
- BƯỚC 2: 
  ...
  + ⛔ NẾU HỌC SINH LÀM SAI: TUYỆT ĐỐI KHÔNG được cho đáp số đúng. Hãy chỉ gợi ý "Gần đúng rồi, bạn thử kiểm tra lại bước... xem sao" hoặc "Bạn thử dùng quy tắc... để tính nhé". Hãy là một người hướng dẫn kiên nhẫn.`;
